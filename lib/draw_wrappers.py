...
def draw_synteny(
    ...
    groups -- a list of which track groups to plot, in the order to be plotted
    data -- a dictionary keyed by track group (ie isolate or genome) with dict as below.
        track_group: {
            'group_name': 'My Genome!',
            'tracks': [[SeqRecord, SeqRecord, ...],  # gene track
                       [np.array, np.array, ...]],  # plot track
            'track_names': ['Genes', 'GC %'],  # track names
            'track_types': [Features, Line],  # Featues, line, hist, heatmap, ...?
            'subtrack_names': [['scaffold_1', 'scaf_2', ...],
                               [None, None, ...]], (optional)
            'xlims': [(start, end), (start, end), ... ], # xlims for axes (optional)
            'ylims': [[(0, 1), (0, 1), ...],  # ylims for axes because ylims are not linked, must specify for all dimensions (optional).
                      [(0, 100), (0, 100), ...]],
            'shapes': {}, (optional) shapes to use for this track only
            'feature_names': None, dict of names to print and kwargs to send to the text instance, default = None (print nothing) optionally provide dict with 'all' key to print 'all' feature names.
            }
